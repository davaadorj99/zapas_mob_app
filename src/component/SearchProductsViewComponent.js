...
import { numberWithComma, logfunction, calculateOffPercentage } from "@helpers/FunctionHelper";
    ...
    for (let s = 0; s < data.length; s++) {
        if (data[s].special != null) {
            ...
            if (startDate <= moment(new Date(), "DD/MM/YYYY") && endDate >= moment(new Date(), "DD/MM/YYYY")) {
                ...
                off = calculateOffPercentage(data[s].price, data[s].special.price) + '% off';
            }
        }
    }
...

                                {
                                    special > 0 ?
                                        <View style={styles.SpcialView}>
                                            ...
                                            <Text style={styles.originalPrice}>{CURRENCY}{numberWithComma(item.price)}</Text>
                                        </View> : <Text style={[styles.price, { flex: 0.70 }]}>{CURRENCY}{numberWithComma(item.price)}</Text>
                                }
                                {
