...
function DealsProduct(props) {
    ...
    if (data.special != null) {
        ...
        off = calculateOffPercentage(data.product_details.price, data.special.price) + '% off';
    }
        ...
        <TouchableOpacity style={styles.productBox} onPress={() => props.navToDetail(data)}>
            <View style={styles.imageView}>
                ...
                <FastImage
                    style={styles.image}
                    source={{
                        uri: data.product_details.image ? ASSETS_DIR + 'product/' + data.product_details.image : ASSETS_DIR + '/assets/img/default.png',
                ...
                <View style={styles.priceView}>
                    {
                        special > 0 ?
                            <View style={styles.SpcialView}>
                                ...
                                <Text style={styles.originalPrice}>{CURRENCY}{numberWithComma(data.product_details.price)}</Text>
                            </View> : <Text style={[styles.price, { flex: 0.70 }]}>{CURRENCY}{numberWithComma(data.product_details.price)}</Text>
                    }
                    {
            ...
            {
                wishlistArr && wishlistArr.length > 0 && wishlistArr.includes(data.product_id) ? <TouchableOpacity style={GlobalStyles.FavCircle} onPress={() => props.addToWishlist(data.product_id)} >
                    <Icon name="heart" style={GlobalStyles.FavIcon} color={Colors.white} />
                </TouchableOpacity> : <TouchableOpacity style={GlobalStyles.unFavCircle} onPress={() => props.userAuth ? props.addToWishlist(data.product_id) : props.navToLogin()}>
