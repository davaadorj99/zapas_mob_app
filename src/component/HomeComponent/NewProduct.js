    ...
    const renderCard = item => {
        ...
        return (
            <View style={styles.productBox} key={item.id.toString()} >
                ...
                <ProductView data={item} key={item.id} navToDetail={navigateToDetailPage} navToLogin={navigateToLoginPage} userAuth={props.userAuth} addToWishlist={addToWishlist} wishlistArray={wishlistArr} />
            </View>
        );
    };
    ...
    return (
        <>
            <View style={styles.catHeading}>
                <Text style={GlobalStyles.boxHeading}> ШИНЭ СЭЛБЭГ</Text>
                <TouchableOpacity style={{ flex: 0.50 }} onPress={() => props.navigation.navigate('ProductListScreen', { title: 'New Products', type: 'newProduct' })}>
                    <Text style={GlobalStyles.viewAll}> ХАРАХ</Text>
                </TouchableOpacity>
            </View>
            <OtrixDivider size={'sm'} />
            <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
                {console.log('new product data ----------------',props)}
