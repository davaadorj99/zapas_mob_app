...
function FlatListProductView(props) {
    ...
    var myPhotos=[]
    myPhotos[0]= data.photo
    const myPhoto = myPhotos[0].split(",");
    console.log('myPhoto----------',myPhoto[0])
    ...
    return (
        <TouchableOpacity style={styles.productBox} onPress={() => props.navToDetail(data)}>
             ...
             {console.log('props----navTodetail-----------', props.navToDetail(data))}
            <View style={[styles.imageView, { backgroundColor: props.imageViewBg ? props.imageViewBg : Colors.light_white }]}>
                ...
                <FastImage
                    style={styles.image}
                    source={{
                        uri: data.photo? ASSETS_DIR +  myPhoto[0] : ASSETS_DIR + '/assets/img/default.png',
                ...
                <View style={styles.priceView}>
                    {
                        special > 0 ?
                            <View style={styles.SpcialView}>
                                ...
                                <Text style={styles.originalPrice}>{CURRENCY}{numberWithComma(price)}</Text>
                            </View> : <Text style={[styles.price, { flex: 0.70 }]}>{price} </Text>
                    }
                    {
...

            {
                ...
                // </TouchableOpacity> : <TouchableOpacity style={[GlobalStyles.unFavCircle, { backgroundColor: Colors.light_white }]} onPress={() => props.addToWishlist(data.id)}>
