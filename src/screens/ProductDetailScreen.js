...
function ProductDetailScreen(props) {
    ...
    const [state, setState] = React.useState({ loading: true, productPrice: 0, productCount: 1,productStock:0, productDetail: null, productNew:null, productDelivery:null,productInstallation:null, value: null, isFocus: false, productDescription: null, productSpecial: null, productAttributes: null, productImages: null, productRelated: null, productOption: [], fetchCart: false, productReview: null, optionColor: 0, optionSelect: 0, optionSize: 0, showZoom: false, zoomImages: [], message: null, type: 'error', optionColorPrice: 0, optionSelectPrice: 0, optionSizePrice: 0, });
    const { loading, productDetail,productInstallation, productNew, productDelivery, productOption, productPrice,productStock, fetchCart, productReview, productImages, productAttributes, productDescription, isFocus, productRelated, productSpecial, optionColor, optionSelect, optionSize, productCount, zoomImages, showZoom, msg, optionColorPrice, optionSelectPrice, optionSizePrice, message, type } = state;
    ...
    const showOutofStock = () => {
        ...
        setState({ ...state, message: 'Дууссан', type: 'error' });
    }
    ...
    useEffect(() => {
        ...
        getApi.getData(
            ...
            let images = [];
            let zoomImages = [];
            console.log('productOptions[0].photo-----------',productData.productOptions[0].photo)
            ...
            console.log('productImages-----------',productData.productImages[0].photo)
            console.log('productImages-----------', typeof productData.productImages)
            var myArr=[]
            myArr[0]=productData.productImages[0].photo
            console.log('productImage to array----------', myArr[0]);
            

      

            const myArray = myArr[0].split(",");
            console.log('photo myArray----------',myArray[0])
            ...
            setState({
                ...state,
                productDetail: productData.productOptions[0].title,
                productStock: productData.productOptions[0].stock,

                productInstallation:productData.productOptions[0].installation,
                productDelivery:productData.productOptions[0].delivery,
                productNew:productData.productOptions[0].new,

                productDescription: productData.productOptions[0].summary,
                ...
                loading ? <OtrixLoader /> : <>
                    ...
                    <OtrixContent customStyles={styles.productDetailView}>
                        ...
                        <ScrollView style={styles.childView} showsVerticalScrollIndicator={false}>
                            ...
                            <View style={styles.subContainer}>
                                <Text style={styles.headingTxt}>{productDetail}</Text>
                                <Text style={[styles.stock, {
                                    color: productStock> 0 ? '#5ddb79' : '#fe151b'
                                }]}>{productStock > 0 ? 'Агуулахт' : 'Дууссан'}</Text>
                            </View>
                            <View style={styles.subContainer}>
                                <Text style={styles.headingOption}>Суурилуулттай эсэх:  </Text>
                                <Text style={[styles.stock, {
                                    color: productStock> 0 ? '#5ddb79' : '#fe151b'
                                }]}>{productInstallation}</Text>
                                
                            </View>
                            <View style={styles.subContainer}>
                                <Text style={styles.headingOption}>Хүргэлттэй  эсэх:  </Text>
                                <Text style={[styles.stock, {
                                    color: productStock> 0 ? '#5ddb79' : '#fe151b'
                                }]}>{productDelivery} </Text>
                            </View>
                            <View style={styles.subContainer}>
                                <Text style={styles.headingOption}>Шинэ  эсэх:  </Text>
                                <Text style={[styles.stock, {
                                    color: productStock> 0 ? '#5ddb79' : '#fe151b'
                                }]}>{ productNew}</Text>
                            ...
                            <View style={styles.subContainer}>
                                {
                                    productSpecial > 0 ?
                                        <View style={styles.SpcialView}>
                                            <Text style={styles.price}>{CURRENCY}{numberWithComma(productPrice)} </Text>
                                            <Text style={styles.originalPrice}>{CURRENCY}{numberWithComma(productDetail.price)}</Text>
                                        </View> : <Text style={styles.productPrice}>{numberWithComma(productPrice)}</Text>
                                }
                                ...
                                {/* <View style={styles.starView}>
                                    <Stars
                                        ...
                                        disabled={true}
                                    />
                                    <Text style={styles.reviewTxt}>({productReview != null ? productReview.totalReview : 0} Reviews)</Text>
                                </View> */}
                            </View>
                            <OtrixDivider size={'md'} />
                                ...
                                {/* <View style={styles.heartIconView}>
                                    {
                                        ...
                                        </TouchableOpacity> Захиалах: <TouchableOpacity style={[GlobalStyles.unFavCircle, { left: wp('4%'), top: 0 }]} onPress={() => props.USER_AUTH ? addToWish(productDetail.id) : props.navigation.navigate("LoginScreen")}>
                            ...
                            <Text style={[styles.headingTxt, { fontSize: wp('3.8%') }]}>Тайлбар</Text>
                            <OtrixDivider size={'sm'} />
                            {/* <RenderHtml
                                contentWidth={width}
                                source={{
                                    html: productOptions.description
                                }}
                            ...
                            {/* {Object.keys(productAttributes).length > 0 &&
                                <View>
                                    ...
                                    <View style={GlobalStyles.horizontalLine}></View>
                                    <OtrixDivider size={'sm'} />
                                    <Text style={[styles.headingTxt, { fontSize: wp('3.8%') }]}>Specification</Text>
                                    <OtrixDivider size={'sm'} />
                            ...
                            {/* {
                                productRelated.length > 0 && <SimilarProduct navigation={props.navigation} reletedData={productRelated} />
                    ...
                    <View style={styles.footerView}>
                        ...
                        >
                            ...
                            <Text style={GlobalStyles.buttonText}>Захиалах</Text>

                        </Button>
                        <View style={styles.countBox}>
                            <Text style={styles.countTxt} onPress={props.navigation.navigate("LoginScreen")}>{productCount}</Text>
