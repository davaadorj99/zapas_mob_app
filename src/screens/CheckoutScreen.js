        ...
        <OtrixContainer customStyles={{ backgroundColor: Colors.light_white }}>
                                ...
                                addresses.map((item, index) =>
                                    ...
                                    >
                                        ...
                                        {selctedAddress == item.id &&
                                            <Text style={styles.deliveryAddressTxt}>Delivery Address <Icon name="md-checkmark-circle-sharp" color={Colors.themeColor} size={wp('4%')} style={{ textAlignVertical: 'center' }} /></Text>
                                        }
...

            {
                !loading && addresses.length == 0 && <TouchableOpacity style={styles.noaddress} onPress={() => setState({ ...state, showAdd: true })}>
                    <Text style={[styles.addressTxt]} > <MatIcon name="plus" color={Colors.link_color} size={wp('5%')} />Add New Address </Text>
                </TouchableOpacity>
            }
            ...
            {
                !loading && step == 2 && <View style={{ marginHorizontal: wp('4%') }}>
                    <Text style={[styles.summayTitle, { left: wp('1.5%') }]}>Order Comments</Text>
                    <OtrixDivider size={"sm"} />
                    <FormControl isRequired >
                        <Input variant="outline" value={comment}
                            placeholder="Enter Order Comments (optional)" style={GlobalStyles.textAreaInputStyle}
            ...
            <Modal visible={paymentSuccessModal}
                transparent={true}>
                <PaymentSuccessComponent navigation={props.navigation} closePaymentModal={closePay} />
            </Modal>
