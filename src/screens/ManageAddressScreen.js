...
import {
    OtrixContainer, OtrixHeader, OtrixDivider, OtirxBackButton, AddAdressComponent, EditAddressComponent, OtrixLoader, OtrixAlert
} from '@component';
...
function ManageAddressScreen(props) {
    const [state, setState] = React.useState({ addresses: [], showAdd: false, message: null, type: null, countries: [], sumAmount: 0, selectedAddress: null, showEdit: false, editAddressData: [], step: 1, selectedPaymentMethod: 4, paymentSuccessModal: false, loading: true, showAlert: false });
    const [showMessage, setShowLoading] = React.useState(false)
    const { showAdd, addresses, selectedAddress, showEdit, editAddressData, step, countries, loading, message, type, showAlert } = state;
            ...
            getApi.postData(
                ...
                if (response.status == 1) {
                    ...
                    setState({
                        ...
                        selectedAddress: response.addresses.length > 0 ? response.addresses[0].id : 0,
                        loading: false,
                        showAdd: false,
    ...
    const updateAddress = (addressData) => {
        ...
        try {
            getApi.postData(
                'user/editAddress/' + selectedAddress,
                addressData,
            ).then((response => {
                logfunction("RESPONSE ", response)
                if (response.status == 1) {
    ...
    const deleteAddressAlert = (id) => {
        setState({ ...state, showAlert: true, selectedAddress: id, showEdit: false });
    }

    const deleteAddress = () => {
        logfunction("URL ", 'user/deleteAddress/' + selectedAddress)
        getApi.postData(
            'user/deleteAddress/' + selectedAddress,
            [],
        ).then((response => {
    ...
    return (
        <OtrixContainer customStyles={{ backgroundColor: Colors.light_white }}>
                    ...
                    <ScrollView style={styles.addressBox} showsHorizontalScrollIndicator={false} vertical={true}>
                        {
                            addresses.length > 0 && addresses.map((item, index) =>
                                <TouchableOpacity key={index} style={[styles.deliveryBox, {
                                    ...
                                    onPress={() => setState({ ...state, selectedAddress: item.id })}
                                >
                                    ...
                                    <TouchableOpacity style={[styles.editView, { bottom: selectedAddress == item.id ? hp('10%') : hp('10%'), marginVertical: hp("0.5%") }]} onPress={() => deleteAddressAlert(item.id)}>
                                        <Text style={styles.edit}> <MatIcon name="delete" color={Colors.red} size={wp('5%')} /></Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity style={[styles.editView, { bottom: selectedAddress == item.id ? hp('10%') : hp('10%') }]} onPress={() => editAddress(item)}>
            ...
            <Modal visible={showAdd}
                transparent={true}>
                <AddAdressComponent closeAdd={closeAddressModel} addAdress={storeAddress} countries={countries} />
            </Modal>

            {/* Edit Address Screen */}
            <Modal visible={showEdit}
                transparent={true}>
                <EditAddressComponent closeEdit={closeAddressEditModel} editAddress={updateAddress} editData={editAddressData} countries={countries} />
            </Modal>
