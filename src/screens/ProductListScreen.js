...
function ProductListScreen(props) {
    ...
    const fetchData = (ID, page, filterApplied = false, range, fprice = null, frating = null) => {
        ...
        let url = '';
        if (type == 'category') {
            url = 'getProductByCategory/'+ ID
            console.log( 'getProductByCategory---------------', url)
        }
    ...
    const onCategoryClick = (type, value) => {
        ...
        if (value != id) {
            ...
            title = childCategories[index].title;
        }
        ...
        <OtrixContainer customStyles={{ backgroundColor: Colors.light_white }}>
            ...
            <OtrixHeader customStyles={{ backgroundColor: Colors.light_white }}>
                ...
                <View style={[GlobalStyles.headerCenter]}>
                    <Text style={GlobalStyles.headingTxt}>{ console.log( '---data.condition---------------', data)} ssss</Text>
                </View>
                {
            ...
            {/* Horizontal Tag List */}
            {
                ...
                <View style={{ height: hp('6%') }}>
                    <ScrollView style={{ flexDirection: 'row', marginHorizontal: wp('1%') }} horizontal={true} showsHorizontalScrollIndicator={false} >
                        <FilterTags tagName={title} type={'tag'} tagID={id} key={id.toString()} selected={selectedTag} onFilterPress={onCategoryClick} />
                        {
                            childCategories.map((item, index) =>
                                <FilterTags type={'tag'} tagName={item.title} tagID={item.category_id} key={item.category_id.toString()} selected={selectedTag} onFilterPress={onCategoryClick} />
                            )
                        }
                    </ScrollView>
                </View>
                            ...
                            data.length > 0 ?
                                <FlatList
                                    ...
                                    renderItem={({ item, index }) =>
                                        <FlatListProductView data={item} key={item.id} imageViewBg={Colors.white} navToDetail={() => props.navigation.navigate('ProductDetailScreen', { id: item.id })} addToWishlist={() => addToWishlist} wishlistArray={wishlistData} />
                                    }>
            ...
            <Modal visible={filterModelVisible}>
                <FilterComponent selectedFilter={selectedFilters} applyFilter={applyFilter} onFilterPress={filterClick} closeFilter={closeFilterModel} filterPriceVal={filterPrice} filterRatingVal={filterRating} filterPriceRangeVal={filterPriceRange} />
            </Modal>
