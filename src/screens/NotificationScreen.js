...
function NotificationScreen(props) {
    ...
    const [state, setState] = React.useState({ foryouchk: true, categoriesSelectedArr: [], categoryArr: [] });
                ...
                <OtrixDivider size={'sm'} />
                {
                    categoryArr.length > 0 && categoryArr.map((item, index) =>
                        <View style={styles.box}>
                            ...
                            <TouchableOpacity key={index} style={styles.checkboxContent} onPress={() => addToArr(item.category_id)}>
                                {
                                    categoriesSelectedArr.includes(item.category_id) == true ? <Ionicons name="checkbox-marked" color={Colors.themeColor} style={[styles.chkBox, { color: Colors.themeColor, fontSize: wp('6.5%') }]} /> :
                                        <Fontisto name="checkbox-passive" style={styles.chkBox} />
                                }
                            </TouchableOpacity>
                            <Text style={styles.textchk}>{item.category_description.name}</Text>
                        </View>
                    )
                }
